...
**Nessun marker.** La voce resta **viva**: finché la task è attiva si riscrive e si elimina, e preflight, run e il riesame del checkpoint la tengono allineata al codice. Il trasloco nell'inbox della task è del `checkpoint-task`, mai di questa skill: qui il task file è appena nato e `## Doc Impact` è per forza la sede.
