...
**La voce resta viva (clessidra), senza marker.** Finché la task è attiva le voci si riscrivono e si eliminano, e sei autorizzato — anzi tenuto — a farlo sulle voci esistenti quando il codice che stai scrivendo le smentisce: run-task è il momento in cui la clessidra si manutiene, non un archivio ad append. Il trasloco in inbox è del `checkpoint-task`, al rilascio o alla chiusura; dove una nozione atterri lo decide `drain-notions`, in differita.
