...
- **Il criterio è la domanda.** Ogni candidato porta come terzo campo il problema con cui uno ci arriva. Una `SEZIONE` col trattino al posto della domanda **esce**: è prosa che nessuno cercherà, e sta occupando lo spazio di una voce che qualcuno cercherebbe. Un `NOME` o un `ERRORE` col trattino resta in gioco — è già una chiave di ricerca per conto suo, e lo giudichi col criterio che hai sotto. Esce invece qualunque candidato la cui domanda è così generica da valere per mezza documentazione — se la stessa domanda porterebbe ugualmente a dieci altri file, quella voce non discrimina niente.
