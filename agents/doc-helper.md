...
- **Rendi le voci in ordine di merito dentro la propria etichetta**, dalla più importante alla meno. Fra etichette diverse non devi decidere nulla: a valle un passo deterministico taglia ciò che non entra nel tetto, in quest'ordine — sopravvivono per prime la **prima** voce di ogni etichetta di contesto (`ERRORE`, `AREA`, `SINTOMO`), poi **tutti** i `NOME`, e per ultime le voci di contesto dalla seconda in poi. Quello che resta tuo è l'unico giudizio che chi taglia non può dare, cioè quali voci **della stessa etichetta** valgono di più; l'ordine in cui le rendi è quella decisione.
- **Ne discende che la prima voce di ogni etichetta di contesto pesa quanto tutte le altre insieme**: è l'unica che il taglio protegge. La prima `AREA` che rendi è quella che dirà di cosa parla il file quando tutto il resto sarà caduto — mettici il perimetro del documento, non la sezione che ti è parsa più curiosa. Stesso ragionamento per il primo `ERRORE` e il primo `SINTOMO`.
- **Le `AREA` restano poche e sulle parti che nessun nome raggiunge.** Dalla seconda in giù vanno in coda perché sono in parte già dette altrove — nell'indice, accanto alla riga, compaiono il path del file e il suo titolo, che il perimetro lo dichiarano gratis. Un file di sola metodologia non ci perde niente: lì di `NOME` non ce n'è, quindi le sue `AREA` non competono con nulla ed entrano tutte.
